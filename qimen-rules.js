...
const MAINSTREAM_GEJU = [
  ...
  // 以下 2 個格局為 2026-08-29 新增(格局庫擴充第二批)。本次查證環境的網頁抓取工具被組織政策
  // 擋掉大部分命理站台，改用搜尋引擎彙整多篇獨立文章的摘要交叉核對(每一條都額外用精確詞句下
  // 第二次搜尋覆核過用詞是否一致)。同一批查證的「太白入熒/熒入太白」(庚加丙/丙加庚)因發現
  // 不同來源對其吉凶有真正矛盾的解讀(一般事務判凶，但「測賊/測敵」情境下方向相反判吉，屬於
  // 用神/占問類型才會影響的條件式判斷，不是單純吉凶不一致的筆誤)，跟上一批查證時記錄的疑慮
  // 一致(見下方「進行中的擴充計畫」該筆記錄)，這次仍然跳過不收錄，維持只收說法一致的部分。
  {key:'riqi_rudiwang', name:'日奇入地網', sky:'乙', earth:'癸', luck:'凶',
    desc:'乙(日奇)加臨癸，雖然癸水生乙木，仍視為凶格：宜退守隱匿、不宜妄動躁進，遁跡修道、躲災避難反而為吉。'},
  {key:'yinhai_yangmen', name:'陰害陽門', sky:'乙', earth:'戊', luck:'凶',
    desc:'乙加臨戊，利陰人陰事，不利陽人陽事：門逢吉尚可謀為，門逢凶或門迫則破財傷人，公開/陽面的事不利。',
    caveat:'吉凶會隨同宮的門再進一步變化，不是單靠這個天干組合就完全定死。'},
  // 以下 2 個格局為 2026-08-29 新增(格局庫擴充第三批)，查證方式同第二批(搜尋引擎彙整多篇
  // 獨立文章交叉核對，每條額外下第二次精確詞句搜尋覆核)：
  {key:'riqi_rumu', name:'日奇入墓', sky:'乙', earth:'己', luck:'凶',
    desc:'乙(日奇)加臨己，己土代表墓庫，乙木的生機被土掩蓋、前程不明：門凶則事必凶；若同宮逢生門或開門，反而轉為「地遁」吉格，利隱藏意圖、佈局談判、拓展合作。',
    caveat:'這是天盤干加臨地盤干的組合判斷，跟荀爽老師六害體系「三奇入墓」(固定查表，乙一定入墓於坤宮，與這裡同宮是什麼地盤干無關)是兩套不同機制，剛好都用「入墓」這個詞，注意不要混淆。'},
  {key:'riqi_beixing', name:'日奇被刑', sky:'乙', earth:'庚', luck:'凶',
    desc:'乙加臨庚，庚金剋乙木：乙庚本為奇門傳統的夫妻正配，但陰在上、陽在下顛倒了正常次序，故稱「被刑」，主爭訟財產、夫妻/合夥各懷私心、同床異夢，問婚姻感情尤其要注意裂痕。'},
  // 以下 2 個格局為 2026-08-29 新增(格局庫擴充第四批)，查證方式同第二/三批：
  {key:'riqi_rutianluo', name:'日奇入天羅', sky:'乙', earth:'壬', luck:'凶',
    desc:'乙(日奇)加臨壬，壬代表天牢，日奇陷入牢籠般的困境：主尊卑悖亂、上下失序、官司訴訟是非，背後可能有人暗中謀害，情勢還在變化中。'},
  {key:'huobei_ruxing', name:'火悖入刑', sky:'丙', earth:'己', luck:'凶',
    desc:'丙(星奇)加臨己，丙火陷入己土(戌墓)之中：容易招惹是非口舌甚至官司訴訟，好意也可能被私心帶偏而弄巧成拙，吉門尚可維持吉，凶門則轉得更凶。'},
  // 以下 3 個格局為 2026-08-29 新增(格局庫擴充第五批)，查證方式同前幾批。丁加戊/戊加丁是
  // 一組方向相反的格局，名稱不同但都吉，跟「太白入熒/熒入太白」那種吉凶方向真矛盾的情況不同，
  // 第一次搜尋結果內部同時出現「青龍轉光」跟「青龍耀明」兩個名字，額外覆核後確認是分屬丁加戊
  // 跟戊加丁兩個不同組合各自的名稱，不是同一件事的兩種說法，兩個都收錄。
  {key:'qinglong_zhuanguang', name:'青龍轉光', sky:'丁', earth:'戊', luck:'吉',
    desc:'丁加臨戊，青龍(戊)助丁火，主貴人提攜、升遷晉職，一般人也能順遂發展。'},
  {key:'qinglong_yaoming', name:'青龍耀明', sky:'戊', earth:'丁', luck:'吉',
    desc:'戊加臨丁，跟「青龍轉光」方向相反：宜拜見上級、貴人，求名求利吉利。',
    caveat:'若同宮遇入墓或門迫，吉會轉為招惹是非，需先看這宮有沒有命中六害。'},
  {key:'riyue_xianghui', name:'日月相會', sky:'丙', earth:'辛', luck:'吉',
    desc:'丙加臨辛，丙辛相合：謀事能有成就，問疾病則病情不算凶險，利合作、等待時機。',
    caveat:'不利遠行，出行類的事情要另外留意。'},
  // 以下 2 個格局為 2026-08-29 新增(格局庫擴充第六批)，查證方式同前幾批：
  {key:'yunu_xingsha', name:'玉女刑殺', sky:'丁', earth:'庚', luck:'凶',
